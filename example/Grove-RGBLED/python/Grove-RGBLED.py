...
class ChainableLED():
    def __init__(self, clk_pin, data_pin, number_of_leds):
        self.__clk_pin = mraa.Gpio(clk_pin)
        self.__data_pin = mraa.Gpio(data_pin)
        self.__number_of_leds = number_of_leds
 	
	self.__clk_pin.dir(mraa.DIR_OUT)
	self.__data_pin.dir(mraa.DIR_OUT) 
        for i in range(self.__number_of_leds):
            self.setColorRGB(i, 0, 0, 0)
 
    def clk(self):
	self.__clk_pin.write(0)
        time.sleep(0.00002)
	self.__clk_pin.write(1)
        time.sleep(0.00002)
    ...
    def sendByte(self, b):
        ...
        for i in range(8):
            ...
            if (b & 0x80) != 0:
		self.__data_pin.write(1)
            else:
		self.__data_pin.write(0)
            self.clk()
